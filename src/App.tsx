...
export default function App() {
  ...
  const [viewMode, setViewMode] = useLocalStorage<'gui' | 'table'>('passover-view-mode', 'gui')
  ...
  return (
    <div className="min-h-screen bg-[#0F0F1A] text-gray-100 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 relative z-10">
        ...
        >
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <motion.button
              ...
              onClick={() => { pop(); setViewMode(v => v === 'gui' ? 'table' : 'gui') }}
              className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-gray-400 text-sm font-medium hover:bg-white/10 transition-all"
            >
              <span>{viewMode === 'gui' ? '📋' : '🃏'}</span>
              <span>{viewMode === 'gui' ? 'תצוגה טבלאית' : 'תצוגת כרטיסים'}</span>
            </motion.button>
            <motion.div
        ...
        <AnimatePresence mode="wait">
          {viewMode === 'gui' ? (
            <motion.div key={`gui-${activeTabId}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"
            >
              {currentRooms.sort((a, b) => a.order - b.order).map((room, i) => (
                <motion.div key={room.id} id={`room-${room.id}`} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}>
                  <RoomCard
                    room={room}
              ...
              >
                <motion.span className="text-5xl mb-2" animate={{ rotate: [0,90,0] }} transition={{ repeat: Infinity, duration: 4, ease: 'easeInOut' }}>+</motion.span>
                <span className="text-lg font-semibold">הוסף חדר</span>
              </motion.button>
            </motion.div>
          ) : (
            <motion.div key={`table-${activeTabId}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <TableView
                rooms={currentRooms}
                tabs={tabs}
        ...
        {/* Task Summary (below rooms, always visible) */}
        <TaskSummary
          rooms={currentRooms}
          activeTasks={currentActiveTasks}
          onScrollToRoom={(roomId) => {
            const el = document.getElementById(`room-${roomId}`)
            if (el) el.scrollIntoView({ behavior: 'smooth', block: 'center' })
            if (viewMode !== 'gui') setViewMode('gui')
          }}
        />
